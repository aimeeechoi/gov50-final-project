...
library(tidyverse)
library(readxl)
job_postings <- read_excel("~/Desktop/job_postings.xlsx")

remote_or_not <- job_postings |>
  mutate(remote_allowed = if_else(is.na(remote_allowed), "No Remote", "Remote Allowed")) |>
  group_by(remote_allowed) |>
  drop_na(applies, views)

remote_or_not |>
  select(job_id, title, applies, views, remote_allowed)

```

```{r}
plot1 <- remote_or_not |>
  mutate(app_per_view = applies / views) |>
